...
class MainWindow(QMainWindow):
    ...
    def _save_json(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Tasks to JSON", "", "JSON Files (*.json);;All Files (*)"
        )
        if file_path:
            try:
    ...
    def _load_json(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Load Tasks from JSON", "", "JSON Files (*.json);;All Files (*)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            unfinished_tasks = [t for t in loaded_tasks if not t.get("done", False)]
            ...
            QMessageBox.information(self,"Loaded",f"Loaded {len(unfinished_tasks)}  tasks."
)
        except Exception as e:
